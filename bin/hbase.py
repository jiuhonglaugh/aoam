...
from utils import file_util, xml_util, exeCmd
...
def getHbaseXml():
    hbaseXml = {}
    tmp = xml_util.getXml('hbase-site.xml', 'property')
...
def startHbase(host, serverName):
    ...
    if 'hmaster'.find(serverName) >= 0:
        ...
        log.warn('开始启动 {host}  {serverName} 服务'.format(host=host, serverName=serverName))
